...
def pseudo_label_testing(atk_receiver,victim_receiver,UAP_form):
		mat_name_save = f'Unsupervised_labelForm_{UAP_form}_atkRx_{atk_receiver}_vicRx_{victim_receiver}.mat'
		UAP_PATH = {
				'full_label':{},
				'pseudo_label':{},
				}
		...
		psr_range,acc = UAPTest(
				...
				# psr_range = np.linspace(0,0.003,10),
				psr_range=np.linspace( 0, 0.1, 20 ),
				use_Guas = True,
				**UAP_PATH[UAP_form]
				)
		Guassian = {}
		Guassian['acc'] = copy.deepcopy(acc["Guassian_noise"])
		Guassian['psr'] = psr_range
		g_path = os.path.join('resultsMat/Pub_results',f'Widar_atk_Guassian_victim_{victim_receiver}.mat')
		...
		for key in list( acc.keys( ) ):
			acc_buf.append( acc[ key ] )
		min_idx = np.asarray( acc_buf ).mean( axis = 1 ).argmin( )
		acc_all[ 'acc' ] = np.asarray( acc_buf )[ min_idx ]
		acc_all['psr'] = psr_range
