...
from Test_UAP import save_UAP,genereate_UAP,UAPTest
...
for v_model in os.listdir(Victim_model_folder):
    if 'lab' in v_model and 'pgd' in v_model and 'resnet' not in v_model:
            ...
            name = f'Surrogate_Normal_home_atk_lab_PSR_{v_psr}_iter_{v_iter}'
            net = AlexNetTF( config )
            model = net.buildModel( choice = 'defult')
...
# for psr in [3e-3,5e-3]:
#     for n_iter in [4,6,8,10,16]:
#         if psr == 3e-3 and n_iter in [10]:
#             continue
#         config.DNN_name = 'defult'
#         config.epoch = 1000
#         method = 'pgd'
